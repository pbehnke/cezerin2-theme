define("./workbox-83223c85.js", ["exports"], function (e) {
  "use strict"
  try {
  ...
  const d = (e, t) => e.filter(e => t in e),
    ...
    q = g,
    v = async ({ request: e, fetchOptions: t, event: n, plugins: r = [] }) => {
      if (
      ...
      try {
        ...
        throw new s("plugin-error-request-will-fetch", { thrownError: e })
      }
      const c = e.clone()
      try {
        let s
  ...
  class N {
    ...
    async p({
      ...
      if (
        ...
        throw new s("bad-precaching-response", { url: t, status: u.status })
  ...
  ;(e.CacheFirst = class {
    ...
    (e.NetworkFirst = class {
      ...
      async handle({ event: e, request: t }) {
        ...
        const o = this.L({ timeoutId: i, request: t, event: e, logs: n })
        r.push(o)
        let c = await Promise.race(r)
        if ((c || (c = await o), !c)) throw new s("no-response", { url: t.url })
        return c
      }
